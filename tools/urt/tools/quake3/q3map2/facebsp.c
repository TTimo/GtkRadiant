...
//This isn't right but I'm just hacking. 27
static float GetSurfaceAreaOfWinding( winding_t* Winding ){
	float total = 0;
	int j;
	//Gather the points of a triangle
	for ( j = 0; j < Winding->numpoints - 2; j++ )
	{
		vec3_t *tri[3];
		vec3_t SideA;
		vec3_t SideB;
		...
		//Side A
		VectorSubtract( tri[0],tri[1],SideA );
		//Side B
		VectorSubtract( tri[0],tri[2],SideB );

		total += ( VectorLength( SideA ) * VectorLength( SideB ) ) / 2;
	}
