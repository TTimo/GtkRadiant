...
static float GetSurfaceAreaOfTriangle( vec3_t A,vec3_t B,vec3_t C ){

	vec3_t SideA;
	vec3_t SideB;

	//Side A
	VectorSubtract( A,B,SideA );
	//Side B
	VectorSubtract( A,C,SideB );

	return ( VectorLength( SideA ) * VectorLength( SideB ) ) / 2;
...
void InsertModel( char *name, int frame, m4x4_t transform, remap_t *remap, shaderInfo_t *celShader, int eNum, int castShadows, int recvShadows, int spawnFlags, float lightmapScale ){
	...
	for ( s = 0; s < numSurfaces; s++ )
	{
		...
		//Caulk Shader
		caulk = ShaderInfoForShader( "textures/common/caulk" );
			...
			for ( i = 0; i < ds->numIndexes; i += 3 )
			{
				...
				if ( PlaneFromPoints( plane, points[ 0 ], points[ 1 ], points[ 2 ] ) ) {
					...
					if ( PlaneFromPoints( pa, points[ 2 ], points[ 1 ], backs[ 1 ] ) &&
						...
						//buildBrush->sides[ j ].shaderInfo = caulk;

						UseForBsp = qfalse;

						//if (i%10==0) UseForBsp=qtrue;
						//Do a heuristic - nearly axial
						if ( ( fabs( plane[0] ) > 0.95 ) || ( fabs( plane[1] ) > 0.95 ) || ( fabs( plane[2] ) > 0.95 ) ) {
							// UseForBsp=qtrue;
						}


						//Plane is 2 axises only
						//            if (( fabs(plane[0])<0.1) ||(fabs(plane[1])<0.1)||(fabs(plane[2])<0.1))
						//      {
						//              UseForBsp=qtrue;
						//        }
						...
						//Skip any tiny tris
						surfsize = GetSurfaceAreaOfTriangle( points[ 0 ], points[ 1 ], points[ 2 ] );
						if ( surfsize < 8096 ) {
							UseForBsp = qfalse;
						}
						...
						//If this face is going to be part of the structural bsp
						if ( UseForBsp == qtrue ) {
							buildBrush->detail = qfalse;

							//buildBrush->sides[ j ].compileFlags|=~C_SKIP;
							for ( j = 1; j < buildBrush->numsides; j++ )
								buildBrush->sides[ j ].compileFlags |= C_SKIP;
