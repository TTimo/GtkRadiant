...
static int AddMetaTriangleToSurface( mapDrawSurface_t *ds, metaTriangle_t *tri, qboolean testAdd ){
	...
	//if (ds->surfaceNum!=tri->surfaceNum) return 0;
	/* test the triangle */
...
static int CompareMetaTriangles( const void *a, const void *b ){
	...
	/*27: then surface.. */
	if ( ( (metaTriangle_t*) a )->surfaceNum < ( (metaTriangle_t*) b )->surfaceNum ) {
		return 1;
	}
	else if ( ( (metaTriangle_t*) a )->surfaceNum > ( (metaTriangle_t*) b )->surfaceNum ) {
		return -1;
	}
...
void MergeMetaTriangles( void ){
	...
	/* note it */
	Sys_FPrintf( SYS_STD, "--- MergeMetaTriangles ---\n" );
	...
	for ( i = 0, j = 0; i < numMetaTriangles; i = j )
	{
		...
		if ( j <= i ) {
			for ( j = i + 1; j < numMetaTriangles; j++ )
			{
				...
				if ( head->si != end->si || head->fogNum != end->fogNum ||  head->surfaceNum != end->surfaceNum || counter++ > toomany ) {
					break;
				}
			}
		}
